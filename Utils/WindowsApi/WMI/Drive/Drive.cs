    ...
    public class Drive
    {
        ...
        public static bool IsDriveOnExternalDisk(string driveLetter)
        {
            ...
            foreach (ManagementObject drive 
                in new ManagementObjectSearcher("select DeviceID, MediaType,InterfaceType from Win32_DiskDrive")
                .Get()
                .Cast<ManagementObject>())
            {
                ...
                foreach (ManagementObject partition in partitionCollection.Cast<ManagementObject>())
                {
                    if (partition != null)
                    {
                        ...
                        foreach (ManagementObject logical in logicalCollection.Cast<ManagementObject>())
                        {
                            if (logical != null)
                            {
